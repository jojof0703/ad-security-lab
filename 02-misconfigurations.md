...
**Note on passwords:** every account in this lab was set to the same password (`ChangeMe123!`) for setup convenience. This lab isn't an attempt to guess, spray, or brute-force credentials. The goal of this lab was to confirm that the planted misconfigurations were actually exploitable end-to-end, not to test password strength. Using a known password let each hash crack instantly, keeping the focus on whether the underlying misconfiguration could be abused at all, rather than how long offline cracking would take against a stronger one.
